...
(define-values (host api-url resource-url executor-url) (select "150"))
(define http:api-query
  ; get method
  (lambda ((alist '()))
    (query api-url alist)))

(define http:api-create
  ; post method
  (lambda (fields files)
    (create api-url fields files)))

(define http:api-update
  ; put method
  (lambda (id fields files)
    (update api-url id fields files)))

(define http:api-delete
  ; delete method
  (lambda (alist)
...
;;; executor
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
(define http:executor-query
  ; get method
  (lambda ((alist '()))
    (query executor-url alist)))

(define http:executor-create
  ; post method
  (lambda (fields files)
    (create executor-url fields files)))

(define http:executor-update
  ; put method
  (lambda (id fields (files null))
    (update executor-url id fields files)))

(define http:executor-delete
  ; delete method
  (lambda (alist)
...
(define (resource-create channel #:force_update (force-update "true") #:current_version (current-version "true") #:log (log "init submit") #:version (version "2.0"))
  (let* ([response (http:resource-query `(("sdk_id" ,channel)))]
         [inner-version (if (string=? "fail" (hash-ref response 'status))
                            "2.0"
                            (let* ([resource-table (hash-ref response 'resources)]
                                   [newest-version (get-res-version-code resource-table)]
                                   [big-version (substring newest-version 0 2)]
                                   [small-version (number->string (+ 1 (string->number (substring newest-version 2))))])
                              (format "~a~a" big-version small-version)))]
    ...
    (system*/exit-code 7z "a" resource-zip (string-append resource "/*"))
    (transfer 'create 'resource channel `((force-update ,force-update) (current-version ,current-version) (log ,log) (inner-version ,version)))))
...
(define (executor-create channel #:resource-version (resource-version "2.0") #:api-version (api-version "1.1") #:log (log "init submit") #:version (version "1.1.2.0"))
  (let* ([api (number->string (get-api-id (get-api-table (http:api-query `(("api_version" ,api-version))))))]
         ...
         [executor-version (if (string=? "fail" (hash-ref response 'status))
                               (format "~a.~a.0" api-version (substring resource-version 0 1))
                               (let* ([executor-table (hash-ref response 'executors)]
                                      [newest-version (get-executor-version-code executor-table)]
                                      [big-version (substring newest-version 0 6)]
                                      [small-version (number->string (+ 1 (string->number (substring newest-version 6))))])
                                 (format "~a~a" big-version small-version)))])
    (displayln version)
    (transfer 'create 'executor channel `((api ,api) (resource ,resource-id) (log ,log) (executor-version ,version)))))
